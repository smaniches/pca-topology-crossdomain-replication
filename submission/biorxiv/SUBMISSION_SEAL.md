# bioRxiv Submission Seal

This file records the non-self-referential seal for the initial bioRxiv submission package.
...
committed. Regenerating them after any future edit to these files (including this seal file) is
the responsibility of whoever makes that edit; the repository's `checksums.sha256` is the
authoritative, continuously-regenerated record for all tracked files, including these.

## Self-reference policy

This file does not contain its own SHA-256. Its hash is recorded only in the repository-level
`checksums.sha256`, according to the repository's existing checksum convention.

`checksums.sha256` does not contain its own SHA-256.

## Commit and release seal
...
`bioRxiv initial submission package`

## Zenodo

Zenodo DOI:

[Zenodo DOI URL to be inserted after release]

No Zenodo DOI exists yet (verified: no GitHub releases/tags exist as of package-creation time,
and no Zenodo-GitHub integration has been set up). This is a placeholder, not a fabricated value.
