...
\item \textbf{Residualization control.} The tumor/normal class-mean-shift direction is estimated and projected out of the feature matrix (residualization), and the $H_1^{\text{stat}}$ test is repeated on the residualized data. A classifier trained to recover tumor/normal status from the residualized data is also evaluated; its cross-validated AUC collapsing toward chance is evidence the confound direction has been successfully removed without also removing the topological signal.
...
This protocol is applied identically across all four cohorts in the completed program design; at the time of writing it has been executed to completion only on GSE81089. \TODO{confound-audit generalization results for GSE146889, CPTAC-CCRCC, and TCGA-LUAD --- to be inserted by synthesis phase once available. GSE146889's own replication report already surfaces a specific reason the pilot's finding may not transfer: its dominant $H_1$ loop is significantly enriched for tumor-status samples (Fisher's exact $p=2.9\times10^{-9}$), unlike the pilot's small, non-enriched loop --- this audit's outcome in that cohort is genuinely open, not assumed.}
